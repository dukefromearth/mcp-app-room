...
async function fetchHostConfig(): Promise<HostConfig> {
  try {
    ...
    return {
      ...
      roomId: data.roomId ?? "demo",
    };
  } catch {
    return {
      roomdUrl: "http://localhost:8090",
      roomId: "demo",
    };
  }
}
...
function HostEntry({ hostConfigPromise }: HostEntryProps) {
  ...
  const roomdUrl = queryParams.roomd ?? hostConfig.roomdUrl;
  const roomId = queryParams.room ?? hostConfig.roomId;
